...
Entries that include whitespace will be split into multiple entries.
...
When literal whitespace is present in a given URL entry, that entry is split into multiple entries; this empowers the plugin to support a list of hosts provided by environment or keystore variable.
